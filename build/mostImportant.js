...
function Home() {
    return (<>
    <NavBar name="joseph" passedata={post.posts}/>
   </>);
}
exports.default = Home;
const post = { posts: [{ slug: "hey", title: "da vin cicode" }, { slug: "hllo", title: "brief history of time" }] };
const arrayofobjects1 = { adhar: "21323", passportnumber: 222 };
const object1 = { data: [{ name: "joseph a joseph", id: 23 }, { name: "rosily", id: 22 }] };
function NavBar(props) {
    return (<>
    <nav className="px-6 py-2 bg-white shadow-md md:flex ">
        <div className="flex justify-between items-center">
            <div className="text-1xl text-gray-800 font-bold hover:text-gray-800">
                <a href="#">name:{props.name} and the  id is </a>
            </div>
        ...
        <div className="w-full pb-2 md:flex md:items-center md:justify-between md:pb-0">
            <div className="flex flex-col px-2 md:flex-row">
              ...
              {props.passedata.map((data222) => { return (<a href="#" key={data222.title} className="py-2 px-2 text-gray-800 rounded hover:bg-gray-900 hover:text-gray-100 hover:font-medium md:mx-2">{data222.title} <br />{data222.slug}</a>); })}
              
               
            </div>
            <div className=" flex item-center">
                <input type="text" className="w-full px-4 py-3 mx-4 leading-tight text-sm text-gray-400 bg-gray-900 rounded placeholder-gray-200 focus:outline-none focus:shadow-outline" placeholder="search"/>
